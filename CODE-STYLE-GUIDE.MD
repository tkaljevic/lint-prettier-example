...
const MAX_count = 100;
const MAX_RETRY_COUNT = 2;  // SCREAMING_SNAKE_CASE — not allowed for properties/variables
...
2. Angular stuff (if any) - inputs, outputs, viewChild, etc.
3. Public properties (no keyword)
4. Protected properties (protected keyword) - if any
5. Private properties (# prefix)
6. Constructor (if needed)
7. Lifecycle hooks (in components)
8. Public methods (no keyword in services, protected in components)
9. Private methods (# prefix)

**Note:** Properties that are readonly must be explicitly marked with the `readonly` keyword.
...
Avoid writing code directly in constructors. Constructors should remain empty or only contain method calls when necessary. For initialization logic:

1. **In services**: Extract logic into a private method and call it from the constructor if needed
2. **In components**: Extract logic into a private method and call it from `ngOnInit`

**Exception**: Signal effects (using `effect()`) should be defined in constructors, as they need to be created in an injection context.
...
export class UserService {
	...
	constructor() {
		// Don't put business logic directly in constructor
...
export class UserListComponent implements OnInit {
	...
	constructor() {
		// Don't put initialization logic in constructor
...
```ts
✔️ Good - Effects are allowed in constructors
export class UserListComponent {
	#userService = inject(UserService);
	selectedUserId = signal<number | null>(null);

	constructor() {
		// Effects need injection context, so they belong in constructor
		effect(() => {
